...
class LatentDiffusion(DDPM):
    ...
    def on_train_start(self):
        """Record the active config in the configured Lightning logger."""
        experiment = getattr(self.logger, "experiment", None)
        experiment_config = getattr(experiment, "config", None)
        if experiment_config is None:
            return

        with open(os.path.join(self.logdir, "configs", "lightning.yaml"), "r") as file:
            lightning_config = yaml.safe_load(file)
            experiment_config.update(lightning_config, allow_val_change=True)

        with open(os.path.join(self.logdir, "configs", "model.yaml"), "r") as file:
            model_config = yaml.safe_load(file)
            experiment_config.update(model_config, allow_val_change=True)
