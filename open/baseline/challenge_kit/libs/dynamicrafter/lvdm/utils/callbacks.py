...
import torch
from video_utils.image import revert_preprocess_images
...
class ImageLogger(Callback):
    def __init__(
        ...
        if max_logged_images is None:
            self.max_logged_images = max_images
        else:
            self.max_logged_images = max_logged_images

    def set_logger(self, logger):
        self.logger = logger

    def prepare_eval_metrics(self, pl_module):
        from video_utils.metrics import FID, FVD, LPIPS, MSE, PSNR, SSIM, ActionAccuracy
        ...
        if self.action_classifier_ckpt is not None:
            ...
            classifier_module = ActionPredictor.load_from_checkpoint(
    ...
    def log_media(self, pl_module, batch_logs, split):
        """Prepare media tensors for local metric logging.
