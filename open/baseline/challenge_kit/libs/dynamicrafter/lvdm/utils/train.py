...
def prepare_logger(lightning_config, config, global_rank, now):
    logger_params = lightning_config.trainer.logger.params
    if "name" in logger_params:
        logger_params.name = config.name
    if "group" in logger_params:
        logger_params.group = config.group
...
def get_trainer(lightning_config, trainer_config, config, args, workdir, ckptdir, logger):
    ...
    strategy_cfg = trainer_config.get("strategy", get_trainer_strategy(lightning_config))
