...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        self._device = "cuda"
        # Keep the eager (non-TensorRT) path valid even when callers deliberately
        # skip post_initialize() to avoid first-run torch.compile overhead.
        ...
        if not config.skip_component_loading:
            ...
            if dit_dir is not None:
                ...
                if os.path.exists(safetensors_index_path):
                    ...
                    # Load each shard directly into the allocated module.  The
                    # upstream implementation accumulated every F32 shard in a
                    # single dictionary first, adding roughly 66 GiB to host RAM
                    # on top of the already allocated 14B model.
                    unexpected_keys_accum = set()
                    for shard_file in sorted(set(index["weight_map"].values())):
                        shard_path = os.path.join(dit_dir, shard_file)
                        print(f"Loading shard: {shard_path}")
                        shard_state_dict = load_file(shard_path)
                        _, shard_unexpected = self.model.load_state_dict(
                            shard_state_dict, strict=False
                        )
                        unexpected_keys_accum.update(shard_unexpected)
                        del shard_state_dict

                    if unexpected_keys_accum:
                        print(
                            "Unexpected keys when loading pretrained weights: "
                            f"{sorted(unexpected_keys_accum)}"
                        )
                    print("Successfully loaded pretrained weights (streamed shards)")
    ...
    def lazy_joint_video_action(
        ...
        condition_action: torch.Tensor | None = None,
    ) -> BatchFeature:
        ...
        if condition_action is not None:
            expected_action_shape = noise_action.shape
            if condition_action.shape != expected_action_shape:
                raise ValueError(
                    f"condition_action must have shape {expected_action_shape}, "
                    f"got {condition_action.shape}"
                )
            condition_action = condition_action.to(
                device=noise_action.device, dtype=noise_action.dtype
            )
            if condition_action.min() < -1.0 or condition_action.max() > 1.0:
                raise ValueError("condition_action must be normalized to [-1, 1]")
        ...
        for index, current_timestep in enumerate(sample_scheduler.timesteps):
            ...
            if condition_action is not None:
                # Flow-matching inpainting for a known clean action trajectory.
                # Training constructs x_sigma=(1-sigma)*action+sigma*noise.  Rebuild
                # that exact marginal at every video denoising step, keeping the
                # known action endpoint fixed while the video remains generative.
                action_sigma = sample_scheduler_action.sigmas[index].to(
                    device=noise_action.device, dtype=noise_action.dtype
                )
                noisy_input_action = (
                    (1.0 - action_sigma) * condition_action
                    + action_sigma * noise_action
                )
        ...
        latents = noisy_input
        latents_action = (
            condition_action if condition_action is not None else noisy_input_action
        )
        output = latents
