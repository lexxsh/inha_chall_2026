...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        if os.environ.get("DIFFSYNTH_REDIRECT_COMMON_FILES", "true").lower() == "false":
    ...
    def __call__(
        ...
        track_control: torch.Tensor = None,
        motion_field_control: torch.Tensor = None,
        ...
        inputs_shared = {
            ...
            "motion_field_control": motion_field_control,
            "end_image": end_image,
...
def model_fn_wan_video(
    ...
    motion_field_control: Optional[torch.Tensor] = None,
    **kwargs,
):
    ...
    x = latents
    # Keep the image-conditioning latent separate before it is concatenated to
    # the noisy video.  The SO100 spatial action adapter uses this tensor to
    # localize control relative to the source image.
    image_condition_latents = y
    # Merged cfg
    ...
    if hasattr(dit, "wantodance_enable_global") and dit.wantodance_enable_global and int(wantodance_fps + 0.5) != 30:
        x = dit.patchify(
            x, control_camera_latents_input, track_control, enable_wantodance_global=True
        )
    else:
        x = dit.patchify(x, control_camera_latents_input, track_control)
    ...
    # Robot action conditioning.  Legacy TI2V adapters modulate token-wise
    # AdaLN.  Wan2.1 I2V can instead construct a source-aligned spatial
    # residual which is injected at multiple transformer depths below.
    spatial_action_residual = None
    action_kv = None
    if actions is not None:
        spatial_adapter = getattr(dit, "spatial_action_adapter", None)
        action_token_encoder = getattr(dit, "action_token_encoder", None)
        if action_token_encoder is not None:
            # Cross-attention (version="xattn"): actions become shared key/value
            # tokens that every DiTBlock attends to via its own gated query.
            action_kv = action_token_encoder(actions.to(device=x.device, dtype=x.dtype))
        elif spatial_adapter is not None:
            if image_condition_latents is None:
                raise RuntimeError(
                    "Spatial actions require first-frame VAE conditioning latents"
                )
            source_dim = spatial_adapter.source_dim
            if image_condition_latents.shape[1] < source_dim:
                raise RuntimeError(
                    f"Image conditioning has {image_condition_latents.shape[1]} channels, "
                    f"but spatial action adapter needs {source_dim}"
                )
            source_latents = image_condition_latents[:, -source_dim:, :1]
            spatial_action_residual = spatial_adapter(
                actions.to(device=x.device, dtype=x.dtype),
                source_latents.to(device=x.device, dtype=x.dtype),
                (f, h, w),
            )
        else:
            if getattr(dit, "action_conditioner", None) is None:
                raise RuntimeError("actions were provided but Wan action conditioning is not enabled")
            action_mod = dit.action_conditioner(
                actions.to(device=x.device, dtype=x.dtype),
                latent_frames=f,
                spatial_tokens=h * w,
            )
            if action_mod.shape[0] != t_mod.shape[0] and t_mod.shape[0] != 1:
                raise ValueError(
                    f"action/timestep batch mismatch: {action_mod.shape[0]} vs {t_mod.shape[0]}"
                )
            t_mod = t_mod + action_mod.to(device=t_mod.device, dtype=t_mod.dtype)

    oracle_motion_hidden = None
    if motion_field_control is not None:
        motion_adapter = getattr(dit, "oracle_motion_field_adapter", None)
        if motion_adapter is None:
            raise RuntimeError(
                "motion_field_control was provided but oracle motion conditioning is not enabled"
            )
        if image_condition_latents is None:
            raise RuntimeError(
                "Oracle motion fields require first-frame VAE conditioning latents"
            )
        source_dim = motion_adapter.source_dim
        if image_condition_latents.shape[1] < source_dim:
            raise RuntimeError(
                f"Image conditioning has {image_condition_latents.shape[1]} channels, "
                f"but oracle motion adapter needs {source_dim}"
            )
        source_latents = image_condition_latents[:, -source_dim:, :1]
        oracle_motion_hidden = motion_adapter(
            motion_field_control.to(device=x.device, dtype=x.dtype),
            source_latents.to(device=x.device, dtype=x.dtype),
            (f, h, w),
        )
    ...
    if use_unified_sequence_parallel:
        if dist.is_initialized() and dist.get_world_size() > 1:
            ...
            if spatial_action_residual is not None:
                control_chunks = torch.chunk(
                    spatial_action_residual, get_sequence_parallel_world_size(), dim=1
                )
                control_chunks = [
                ...
                spatial_action_residual = control_chunks[get_sequence_parallel_rank()]
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vap is not None and block_id in vap.mot_layers_mapping:
                ...
                x = gradient_checkpoint_forward(
                    ...
                    x, context, t_mod, freqs, action_kv
                )

            if spatial_action_residual is not None:
                block_residual = dit.spatial_action_adapter.residual_for_block(
                    spatial_action_residual, block_id
                )
                if block_residual is not None:
                    x = x + block_residual

            if oracle_motion_hidden is not None:
                block_residual = dit.oracle_motion_field_adapter.residual_for_block(
                    oracle_motion_hidden, block_id
                )
                if block_residual is not None:
                    x = x + block_residual
