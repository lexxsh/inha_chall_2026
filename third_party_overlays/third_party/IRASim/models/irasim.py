...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t, use_fp16=False):
        ...
        # The released code only handled FP32/FP16.  Our target uses BF16, so
        # the sinusoidal embedding must follow the MLP parameter dtype rather
        # than a boolean FP16 flag; otherwise BF16 inference reaches Linear
        # with a Float input and fails before the first denoising step.
    ...
    def forward(self, 
        ...
        if self.extras == 3:
            ...
            if self.training:
                ...
                # An indexed in-place write can invalidate saved tensors in
                # recent PyTorch autograd. This is algebraically identical.
                state_embeddings = torch.where(
                    mask[:, None, None], mask_emb_expanded, state_embeddings
                )
            ...
            if self.training:
                ...
                state_embeddings = torch.where(
                    mask[:, None], mask_emb_expanded, state_embeddings
                )
