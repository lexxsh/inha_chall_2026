...
def _build_edge_policy_metadata(training_config: Any) -> dict[str, Any]:
    ...
    outer = _config_value(training_config, "dataloader_train")
    legacy_loaders = _config_value(outer, "dataloaders")
    action_data = _config_value(legacy_loaders, "action_data")
    legacy_dataloader = _config_value(action_data, "dataloader")
    legacy_dataset = _config_value(legacy_dataloader, "dataset")
    dataset_entries = list(_config_value(legacy_dataset, "list_of_datasets") or [])
    if not dataset_entries:
        # Current action-SFT recipes use PackingDataLoader ->
        # RankPartitionedDataLoader -> datasets rather than the legacy joint
        # dataloader layout above.  Preserve both official config shapes.
        inner = _config_value(outer, "dataloader")
        datasets = _config_value(inner, "datasets")
        dataset_entries = list(datasets.values()) if datasets else []
    if not dataset_entries:
        raise ValueError(
            "Cosmos3 Edge export requires at least one action dataset entry in either the legacy "
            "joint-dataloader or PackingDataLoader config layout."
        )
