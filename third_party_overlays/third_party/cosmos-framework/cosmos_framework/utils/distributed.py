...
def init() -> int | None:
    ...
    try:
        ...
        # NVML reports host CPU ids, while schedulers/containers may expose only
        # a restricted cpuset.  Passing host-only ids to sched_setaffinity raises
        # EINVAL and aborts every distributed worker before NCCL is initialized.
        requested_affinity = set(device.get_cpu_affinity())
        allowed_affinity = set(os.sched_getaffinity(0))
        effective_affinity = requested_affinity & allowed_affinity
        if effective_affinity:
            os.sched_setaffinity(0, effective_affinity)
        else:
            log.warning(
                "Skipping GPU CPU affinity because the NVML CPU set does not "
                "overlap the process cpuset."
            )
