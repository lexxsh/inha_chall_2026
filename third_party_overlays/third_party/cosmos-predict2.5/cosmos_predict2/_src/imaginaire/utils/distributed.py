...
def init() -> int | None:
    ...
    try:
        ...
        # NVML reports host CPU ids, while a Slurm/container cpuset can expose
        # only a subset of those ids. Restrict the request to CPUs this process
        # is actually allowed to use; an empty intersection is safe to skip.
        requested_affinity = set(device.get_cpu_affinity())
        allowed_affinity = os.sched_getaffinity(0)
        usable_affinity = requested_affinity & allowed_affinity
        if usable_affinity:
            os.sched_setaffinity(0, usable_affinity)
        else:
            log.warning(
                "Skipping GPU CPU affinity because NVML and the process cpuset "
                f"do not overlap (rank={local_rank})."
            )
