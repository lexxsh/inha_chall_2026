...
class VQModel(L.LightningModule):
    def __init__(
        ...
        # The released class eagerly constructs the training-only LPIPS and
        # discriminator, which also triggers a VGG download.  Tokenization and
        # reconstruction need neither module.  Keep the official default for
        # training while allowing an exact generator-only audit.
        self.loss = None if inference_only else VQLPIPSWithDiscriminator(config)
